...
def main(config: TrainConfig):
    ...
    action_low, action_high = float(envs.single_action_space.low[0]), float(
    ...
    def get_params_qnet():
        qf1 = td3.QNetwork(ob_space, ac_space, config.device)
        qf2 = td3.QNetwork(ob_space, ac_space, config.device)
        ...
        qnet = td3.QNetwork(ob_space, ac_space, device="meta")
        qnet_params.to_module(qnet)

        return qnet_params, qnet_target_params, qnet

    def get_params_actor(actor):
        target_actor = td3.Actor(ob_space, ac_space, envs=envs, device="meta")
    ...
    actor_optimizer = optim.AdamW(list(actor.parameters()), lr=config.policy_lr, capturable=True)
    ...
    def update_main(data):
        ...
        clipped_noise = (
            clipped_noise.mul(policy_noise).clamp(-noise_clip, noise_clip).mul(action_scale)
        )
    ...
    for global_step in pbar:
        ...
        if global_step < config.learning_starts * 9 / 10:
            ...
            actions = policy(obs=normalized_flat_obs)
        ...
        if global_step > config.learning_starts:
            ...
            for j in range(config.n_updates):
                ...
                log_infos.update(update_main(data))
                ...
                if j % config.policy_frequency == 1:
                    ...
                    target_actor_params.lerp_(actor_params.data, config.tau)
