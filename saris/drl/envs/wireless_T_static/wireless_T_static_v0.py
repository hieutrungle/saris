...
class WirelessTStaticV0(Env):
    ...
    def __init__(
        ...
        log_string: str = "WirelessTStaticV0",
        eval_mode: bool = False,
        seed: int = 0,
        **kwargs,
    ):
        super(WirelessTStaticV0, self).__init__()
    ...
    def _get_action_space(self) -> spaces.Box:
        ...
        r_low = -2.0
        r_high = 2.0
        theta_low = np.deg2rad(-2.0)
        theta_high = np.deg2rad(2.0)
        phi_low = np.deg2rad(-2.0)
        phi_high = np.deg2rad(2.0)
    ...
    def _cal_reward(
        ...
        reward = total_gain + 0.3 * gain_diff - 0.05 * cost_time
