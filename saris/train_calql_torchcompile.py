...
def main(config: TrainConfig):
    ...
    # obs = torch.randn(3, ob_dim, device=config.device)
    # actions, log_prob, mean = actor.get_action(obs)
    ...
    # actions = torch.randn(3, 2, action_dim, device=config.device)
    # obs = extend_and_repeat(obs, dim=1, repeat=2)

    # actions = actions.view(-1, actions.shape[-1])
    # obs = obs.view(-1, obs.shape[-1])

    # q_values = critic_1(obs, actions)
