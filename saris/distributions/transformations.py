...
def is_constant_jacobian(fn, x=0.0):
    ...
    jac_is_constant = not any(
        isinstance(v, jax.core.Var) and v in dependent_vars for v in jac_jaxpr.outvars
    )
...
def _check_numerical_stability(fn):
    ...
    for eqn in jaxpr.eqns:
        if eqn.primitive in _potentially_unstable_primitives:
            logging.warn(
                "[Distrax]: the '%s' primitive can exhibit unstable "
                "numerical behavior under certain circumstances. Consider "
                "using the %s bijector instead if possible.",
                eqn.primitive,
...
def _dependent_variables(jaxpr, dependent=None):
    ...
    for eqn in jaxpr.eqns:
        ...
        if call_jaxpr:
            ...
            dependent.update(
                from_name[v] for v in call_jaxpr.outvars if v in subjaxpr_dependent
            )
        else:
            for v in eqn.invars:
...
def _identify_variable_in_eqn(eqn):
    ...
    if len(eqn.invars) == 1:  # unary operation
        ...
        if tuple(map(type, eqn.invars)) == (jax.core.Var, jax.core.Literal):
            ...
            raise ValueError(
                "Expression appears to contain no variables and "
        ...
        raise NotImplementedError(
            f"Op {eqn.primitive} with cardinality >= 3 not " "supported."
        )
...
def _interpret_inverse(jaxpr, consts, *args):
    ...
    for eqn in reversed(jaxpr.eqns):
        ...
        if call_jaxpr:
            subfuns = [
                lu.wrap_init(functools.partial(_interpret_inverse, call_jaxpr, ()))
            ]
            prim_inv = eqn.primitive

        elif eqn.primitive is jax.experimental.pjit.pjit_p:
            pjit_jaxpr = params.pop("jaxpr")
            partial_inverse = functools.partial(
                _interpret_inverse, pjit_jaxpr.jaxpr, pjit_jaxpr.consts
            )
    ...
    if any(v not in env for v in jaxpr.invars):
        raise ValueError(
            "Expression appears to contain no variables and therefore "
            "cannot be inverted."
        )
