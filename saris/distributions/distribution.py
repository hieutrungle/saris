"""
source: https://github.com/google-deepmind/distrax/blob/master/distrax/_src/distributions/distribution.py
"""
...
# Generic nested type.
NestedT = Union[
    T, Iterable["NestedT"], Mapping[Any, "NestedT"]
...
class Distribution(
    ...
    def survival_function(self, value: EventT) -> Array:
        ...
        if not self.event_shape:
            ...
            raise NotImplementedError(
                "`survival_function` is not defined for " f"distribution `{self.name}`."
            )
    ...
    def log_survival_function(self, value: EventT) -> Array:
        ...
        if not self.event_shape:
            ...
            raise NotImplementedError(
                "`log_survival_function` is not defined for "
...
def to_batch_shape_index(
    ...
    try:
        ...
        raise IndexError(
            f"Batch shape `{batch_shape}` not compatible with index " f"`{index}`."
        ) from e


DistributionLike = Union[Distribution]
